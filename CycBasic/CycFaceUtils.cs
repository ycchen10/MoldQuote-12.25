...
namespace CycBasic
{
    public class CycFaceUtils
    {
        ...
        /// <returns></returns>
        public static CycFaceData AskFaceData(Face face)
        {
            ...
            CycFaceData fd = new CycFaceData();
            fd.FaceType = type;
        ...
        public static double GetFaceMaxDia(Face face)
        {
            ...
            if (face.SolidFaceType == Face.FaceType.Cylindrical || face.SolidFaceType == Face.FaceType.Conical || face.SolidFaceType == Face.FaceType.Planar)
            {
                foreach (Edge edge in face.GetEdges())
                {
                    ...
                    {
                        double radius = CycEdgeUtils.GetArcRadius(edge);
        ...
        {
            Vector3d vec1 = CycFaceUtils.AskFaceNormal(face1);
            Vector3d vec2 = CycFaceUtils.AskFaceNormal(face2);
        ...
        public static void GetSweptSlope(Face sweptFace, Vector3d vec, out double[] slope, out double[] rad)
        {
            ...
            {
                param[0] = i * (uvs[0] + uvs[1]) / accuracy + uvs[0];
                param[1] = i * (uvs[2] + uvs[3]) / accuracy + uvs[2];
